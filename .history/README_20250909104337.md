...
- **仕組み:** AI には `{{image_URL}}` のような特別な目印（プレースホルダー）を含む部分的なパスを出力させます。PWA 側でその目印を、設定画面で指定したベース URL に自動的に置換します。
- **メリット:**
...
- **設定方法:**
  ...
  3.  AI へのプロンプトで、`![alt text]({{image_URL}}/path/to/image.png)` の形式で出力するよう指示します。
- **現状の課題:**
