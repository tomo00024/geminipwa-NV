...
# 派生版 GeminiPWA(ti) の変更点・仕様・連絡事項（2025年7月時点）
*  （⚠️の数は重要度を指します）
*   ⚠️⚠️⚠️⚠️⚠️当システム外の環境変化によるアプリの破損・動作の不安定化が懸念されたため、急遽便宜的に、システムの自己修復能力を強化したバージョンをリリースしました。
試製版“Twin-engine”も実装されていますが、調整中のため使用しないでください。（使用方法を誤ると湯水のようにトークンを浪費する事態になり得ます）2025/07/19
*   ⚠️⚠️⚠️⚠️⚠️過去のセッション履歴やプロンプト等の重要なデータは、各ユーザー様の任意のタイミングでバックアップしていただきますようお願いいたします。 
*   ⚠️⚠️各ボタン類の表示/非表示は設定画面から選択可能です。
*   ⚠️⚠️⚠️“Twin-engine”を近日実装します。AIは通常、記憶できる会話の量に上限があります。会話が長くなると、AIは初期の指示や途中の重要なやり取りを忘れてしまい、話が噛み合わなくなったり、役割を忘れたりします。
* “Twin-engine”は、この問題を解決するために、裏側でもう一体のAI（要約用AI）を動かす仕組みです。これはメインのAIにセッション全体を渡す代わりに“セッションの要約”を渡す機能であり、
* これにより、AIの記憶能力の改善及びAIとの言語ゲームが定型化してしまう事態を防ぐ効能が期待できます。またケースバイケースですが、API利用料を削減する効果もあり得ます。
