...
*    ⚠️⚠️⚠️メインのセッション画面の上部に存在するシステムプロンプトを、今後2週間以内に撤去いたします。この変更により、今後使用可能なシステムプロンプトは設定画面に存在する“共通システムプロンプト”と各 APIプロバイダー毎の“固有システムプロンプト”の2種となります。
...
*   ⚠️過去のセッション履歴やプロンプト等の重要なデータは、各ユーザー様の任意のタイミングでバックアップしていただきますようお願いいたします。
*   ⚠️APIプロバイダーのオプションにLLM Aggregatorを追加しました。openrouter.ai（https://openrouter.ai/api/v1）などのAPIを使用することができます。詳細は追ってご説明申し上げます。
*   ⚠️grok(xAI)のAPIに対応しました。注意点は従前に行った対応API追加アップデートと同様です。使用するAPIプロバイダー:に選択肢として表示されない場合、その他設定> ヘッダー/フッターボタン・項目表示設定>API切替ボタン設定から選択肢を設定してください
