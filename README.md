...
-  質問例: 「このPWAの初めての使い方を教えて」「APIキー・システムプロンプト・ダミープロンプトって何？」「ユーザーデータの扱いはどんな感じ？」

# titan823によるフォーク版 GeminiPWA(ti) の変更点・仕様（2025年6月時点）
*   ⚠️今後の情勢の変化に備えて、ChatGPTへの対応を試みましたが、現時点では公式のサービスのほうが扱いやすいかと思います。試験的な実装であり、単にチャットで利用できるようにしただけの簡易的なものとなっております。
...
- XSS対策セキュリティ（DOMPurifyの導入とmarked.js出力のサニタイズ 、および入力値検証の強化）を施しました。
