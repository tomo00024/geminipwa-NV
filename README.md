...
-  使い方や分からないことがあれば、このリポジトリの「index.html」ファイルの中身をコピーしてGoogle AI Studio等でAIにご提示の上お尋ねください。
-  質問例: 「このPWAの初めての使い方を教えて」「APIキー・システムプロンプト・ダミープロンプトって何？」「ユーザーデータの扱いはどんな感じ？トラッキングされたり情報盗まれたりする？ API料金から公開者にマージン抜かれたりとかは？」

# titan823による派生版 GeminiPWA(ti) の変更点・仕様（2025年6月時点）
*   ⚠️今後の情勢の変化に備えて、ChatGPTへの対応を試みましたが、現時点では公式のサービスのほうが扱いやすいかと存じます。試験的な実装であり、単にチャットで利用できるようにしただけの簡易的なものとなっております。
...
### 【2025/06/11】
-  ChatGPT(OpenAI API)のAPIを利用できるようにアップデート
### 【2025/06/09】
...
- OpenAI API対応前の最後の版（予定）をログとしてブランチ化
### 【2025/06/08】
