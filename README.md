...
*  （⚠️の数は重要度を指します）
*    ⚠️kinkan04氏が公開なさっているPWA(https://github.com/kinkan04/Gemini-PWA-Mk-II)の校正機能が好評のため、こちらのリポジトリのコードにも（仕様をこちらに適合させつつ）移植・導入させていただきました。校正以外にも自動的にテキストを差し替える用途などにも使える大変有用な機能と存じます。有効化する場合、“その他設定→アプリの挙動を調整→上級ユーザー向けオプション→校正機能を使用する”をオンにし、セットアップしてください。2025/08/10
